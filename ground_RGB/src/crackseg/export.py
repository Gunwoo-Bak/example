...
from .geo3d import Geo3DContext, add_geo3d_to_instance_rows, build_geo3d_damage_pixel_rows
...
def save_prediction_outputs(
    ...
    geo3d_context: Geo3DContext | None = None,
    save_damage_pixels_3d: bool = False,
    pixel_xyz_stride: int = 1,
    ...
    damage_pixels_3d_path = spatial_dir / f"{stem}_damage_pixels_3d.csv"
    ...
    add_timing(profile_timings, "export_visual_outputs_sec", time.perf_counter() - visual_start)

    geotiff_start = time.perf_counter()
    geotiff_info = read_geotiff_info(source_path) if source_path is not None else None
    add_timing(profile_timings, "export_geotiff_read_sec", time.perf_counter() - geotiff_start)
    ...
    add_timing(profile_timings, "export_pixel_summary_sec", time.perf_counter() - pixel_start)
    ...
    add_timing(profile_timings, "export_instances_2d_sec", time.perf_counter() - instance_start)

    if geo3d_context is not None:
        geo3d_start = time.perf_counter()
        instance_rows = add_geo3d_to_instance_rows(instance_rows, geo3d_context, pred_mask=pred_mask)
        add_timing(profile_timings, "export_instances_3d_sec", time.perf_counter() - geo3d_start)

    instance_write_start = time.perf_counter()
    ...
    add_timing(profile_timings, "export_instance_csv_write_sec", time.perf_counter() - instance_write_start)

    damage_pixel_rows = []
    if geo3d_context is not None and save_damage_pixels_3d:
        damage_start = time.perf_counter()
        damage_pixel_rows = build_geo3d_damage_pixel_rows(
            pred_mask=pred_mask,
            class_names=config.class_names,
            context=geo3d_context,
            stride=pixel_xyz_stride,
        )
        write_dict_rows(damage_pixels_3d_path, damage_pixel_rows)
        add_timing(profile_timings, "export_damage_pixels_3d_sec", time.perf_counter() - damage_start)
    ...
    metadata: dict[str, Any] = {
        ...
        "geo3d": geo3d_context.to_dict() if geo3d_context is not None else None,
        "damage_pixels_3d": {
            "enabled": bool(save_damage_pixels_3d),
            "stride": int(pixel_xyz_stride),
            "row_count": len(damage_pixel_rows),
            "path": str(damage_pixels_3d_path) if save_damage_pixels_3d else None,
        },
        "note": "No ground-truth mask was provided. Pixel and instance summaries are computed from model predictions only.",
    }
    if profile_timings is not None:
        metadata["profile_timings_sec"] = {key: round(float(value), 6) for key, value in sorted(profile_timings.items())}
    metadata_path.write_text(json.dumps(metadata, ensure_ascii=False, indent=2), encoding="utf-8")
    add_timing(profile_timings, "export_metadata_write_sec", time.perf_counter() - metadata_start)
    ...
    if save_damage_pixels_3d:
        paths["damage_pixels_3d"] = damage_pixels_3d_path
    by_damage_start = time.perf_counter()
    paths.update(
    ...
    add_timing(profile_timings, "export_by_damage_outputs_sec", time.perf_counter() - by_damage_start)
    add_timing(profile_timings, "export_total_sec", time.perf_counter() - export_start)
    return paths
...
def add_timing(timings: dict[str, float] | None, key: str, elapsed_sec: float) -> None:
    if timings is not None:
        timings[key] = timings.get(key, 0.0) + float(elapsed_sec)
